module raw_to_binary (
    ...
    reg [11:0] mem [15:0];
    reg [15:0] sum; //Caution, used to be 16 bits, 12 should be enough
    reg [11:0] avg;
    reg [11:0] avg_w_res;
    ...
    always @(posedge clk or posedge reset) begin
        if (reset) begin
            ...
            sum <= 16'd0;  // Reset sum to 0 before calculation
            for (i = 0; i < 16; i = i + 1) begin
                sum <= sum + mem[i];  // Accumulate the sum of all values in the array
            end
            avg <= sum >>> 4;  // Divide by 16 using arithmetic shift
        end
    end
    ...
    //multiply by resolution (-2g to 2g: res = 1/1024)
    always @(avg) begin
        avg_w_res = avg >>> 10;
    end
    //misses some accuracy
    ...
    always@(avg_w_res) begin
        ...
        signed_binary = (avg_w_res <<< 10) - (avg <<< 4) - (avg <<< 3);

        // Handle the sign: store the sign in the 'sign' register
        is_negative = (signed_binary[12] == 1); // if MSB of 'a' is 1, the number is negative
